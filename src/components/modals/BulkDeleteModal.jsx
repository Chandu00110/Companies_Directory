...
export default function BulkDeleteModal({ open, onClose, onConfirm, count }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md p-6 bg-slate-900 rounded-xl border border-slate-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold">Delete {count} companies?</h3>
        </div>

        <p className="text-slate-300 mb-4">This action will permanently delete {count} selected company(ies). This cannot be undone.</p>
